...
router.get("/stakedTokens", async function (req, res) {
  ...
  if (ids !== "")
    datas = await query(
      "SELECT  Distinct name, gender, token_id, class, initiate_flag, img_url, traits, account_id, baby_count from tb_tokens WHERE `token_id` IN (" +
        ids +
        ")",
      []
    );
...
router.get("/initalize", async function (req, res) {
  for (let i = 24; i < 89; i++) {
    await query(
      "update tb_tokens set class='0', initiate_flag='0' where name = 'NFA baby #" +
        i +
        "'",
      []
    );
