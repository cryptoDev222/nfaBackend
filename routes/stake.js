...
router.get('/stakedList', async function(req, res) {
  ...
  let result = await query("Select stakes.*, tokens.baby_count from tb_stakes as stakes LEFT JOIN tb_tokens as tokens ON stakes.token_id = tokens.token_id where `withdraw` = 0 AND stakes.chain_id=?", chainId)
