...
router.get('/stakeHistory', async function(req, res) {
  let chainId = req.query.chainId
  let result = await query("Select Distinct tb_stakes.*, tb_tokens.name, tb_tokens.img_url from tb_stakes LEFT JOIN tb_tokens ON tb_stakes.token_id = tb_tokens.token_id where tb_stakes.chain_id=? ORDER BY ID DESC", chainId)
